    ...
    <div className="flex min-h-dvh flex-col bg-bg">
      ...
      {auth.user.status === "SUSPENDED" && (
        <div className="container-page pt-4">
          <Alert tone="danger" title={t("Your account is suspended")}>
            {t("You can still read your data, but you cannot list, message or transact. Contact support if you believe this is a mistake.")}
          </Alert>
        </div>
      )}
      ...
      {!auth.user.emailVerified && (
        <div className="container-page pt-4">
          <Alert tone="warning" title={t("Confirm your email address")}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <span>
                {t("You need a confirmed address before you can message sellers, list a pet or buy anything.")}
              </span>
              <div className="shrink-0 sm:w-56">
                <ResendVerification variant="outline" label={t("Resend the link")} />
              </div>
            </div>
          </Alert>
        </div>
